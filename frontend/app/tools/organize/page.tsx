"use client";

import React, { useState } from "react";
import { LayoutGrid, ArrowRight } from "lucide-react";
import FileDropzone from "@/components/FileDropzone";
import JobStatusCard from "@/components/JobStatusCard";
import { executeTool, pollJob, JobResponse } from "@/lib/api";

export default function OrganizePage() {
  const [files, setFiles] = useState<File[]>([]);
  const [pageOrderInput, setPageOrderInput] = useState("");
  const [job, setJob] = useState<JobResponse | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleOrganize = async () => {
    if (files.length === 0) {
      setError("Please select a PDF file to organize.");
      return;
    }
    setError(null);
    setIsLoading(true);

    try {
      let pageOrder: number[] = [];
      if (pageOrderInput.trim()) {
        pageOrder = pageOrderInput
          .split(",")
          .map((s) => parseInt(s.trim(), 10))
          .filter((n) => !isNaN(n) && n > 0);
      }

      const initialJob = await executeTool("organize", files, {
        page_order: JSON.stringify(pageOrder),
      });
      setJob(initialJob);

      const completedJob = await pollJob(initialJob.id, (updated) => setJob(updated));
      setJob(completedJob);
    } catch (err: any) {
      setError(err.message || "Failed to organize PDF.");
    } finally {
      setIsLoading(false);
    }
  };

  const handleReset = () => {
    setFiles([]);
    setPageOrderInput("");
    setJob(null);
    setIsLoading(false);
    setError(null);
  };

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      {/* Header */}
      <div className="text-center space-y-3">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[#141414] border border-[#2a2a2a] shadow-card text-accent mb-2">
          <LayoutGrid className="w-7 h-7" />
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold uppercase tracking-tight text-white">
          Organize & Reorder Pages
        </h1>
        <p className="text-xs sm:text-sm text-neutral-400 max-w-lg mx-auto font-normal">
          Rearrange, duplicate, or omit specific pages by defining your desired page sequence.
        </p>
      </div>

      {!job ? (
        <div className="rounded-2xl bg-[#141414] border border-[#262626] p-6 sm:p-10 shadow-card space-y-6 rivets">
          <FileDropzone
            files={files}
            onFilesChange={setFiles}
            accept="application/pdf"
            multiple={false}
            title="DRAG & DROP PDF HERE"
            description="Select a PDF to rearrange or delete pages"
          />

          {files.length > 0 && (
            <div className="space-y-2 pt-2">
              <label className="block text-xs font-mono font-bold uppercase tracking-wider text-neutral-300">
                Desired Page Order Sequence
              </label>
              <input
                type="text"
                value={pageOrderInput}
                onChange={(e) => setPageOrderInput(e.target.value)}
                placeholder="e.g. 3, 1, 2, 5 (Pages not listed will be removed)"
                className="w-full px-4 py-3 rounded-xl bg-[#0d0d0d] border border-[#262626] text-white text-sm font-mono focus:outline-none focus:border-accent shadow-recessed transition-colors"
              />
              <p className="text-[11px] font-mono text-neutral-500">
                Enter comma-separated 1-based page numbers.
              </p>
            </div>
          )}

          {error && (
            <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs font-mono">
              {error}
            </div>
          )}

          <button
            type="button"
            disabled={files.length === 0 || isLoading}
            onClick={handleOrganize}
            className="w-full btn-accent text-xs py-4 justify-center"
          >
            <span>Reorganize PDF</span>
            <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      ) : (
        <JobStatusCard job={job} isLoading={isLoading} onReset={handleReset} />
      )}
    </div>
  );
}
